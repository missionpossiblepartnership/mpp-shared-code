...
from mppshared.utility.utils import get_unique_list_values, filter_input_metrics
...
def calculate_tech_transitions(
    ...
    Args:
        ...
        ccus_context (): list of all CCU/S OPEX context names that will be included
        ...
        investment_cycle (): Lifetime for computation of cost metrics of all technologies will be set to this value if
            it is not None
        carbon_cost_scopes ():
...
def _get_switch_capex(
    ...
    # set switches from any direct separation CC tech to electric and hydrogen kilns to their renovation CAPEX
    df_brownfield_renovation.loc[
        (
            df_brownfield_renovation["technology_origin"].str.contains(
                "direct separation"
            ...
            & (
                (df_brownfield_renovation["technology_destination"] == "Electric kiln + direct separation")
                ^ (df_brownfield_renovation["technology_destination"] == "Dry kiln + Hydrogen + direct separation")
            )
            & (df_brownfield_renovation["technology_origin"] != df_brownfield_renovation["technology_destination"])
        ),
        "value",
    ] = df_brownfield_renovation["value_destination"]
...
def _get_opex_variable(
    ...
    df_ov_inputs_energy = filter_input_metrics(
    ...
    df_ov_commodity_prices_energy = filter_input_metrics(
    ...
    for cost_classification in dict_ov_ccus.keys():
        # CCU/S OPEX non-process * captured emissivity
...
def _get_lcox(
    ...
    Args:
        ...
        df_capacity_factor (): 
        df_lifetime (): 
        investment_cycle (): Lifetime for all technologies will be set to this value if it is not None
    ...
    # get capacity factor
    df_capacity_factor = filter_input_metrics(
    ...
    # get lifetimes
    df_lifetime = filter_input_metrics(
    ...
    # set lifetime to investment_horizon if given
    if investment_cycle:
        df_lifetime["value"] = investment_cycle
    ...
    dict_capex_opex = {k: None for k in [x for x in dict_capex_opex.keys() if x in ccus_context]}
...
def _opex_get_ccus_context_cost_components(
    ...
    df_ccus_context_cost = filter_input_metrics(
    ...
    for key in dict_ccus_context_cost:
        ...
        dict_ccus_context_cost[key] = filter_input_metrics(
...
def _opex_get_ccus_process_cost(
    ...
    Takes as input a dictionary that includes all the dataframes that are required for computing the CCU/S process cost.
        The function then calculates all cost components, by multiplying two dataframes with each other based on
