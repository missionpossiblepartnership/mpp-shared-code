...
def create_dict_mapper(
    original_df: pd.DataFrame, static_column_list: list, values_to_override: dict
) -> dict:
    ...
    """
    logger.info(
        f"Creating a static value mapper with the following overrides:  {values_to_override}"
    )
...
def full_model_flow(
    ...
    (
        ...
        static_columns,
        last_value,
    ) = create_timeseries_extension_components(
