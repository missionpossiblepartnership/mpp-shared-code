...
def calculate_tech_transitions(
    ...
    if sector == "cement":
        ...
        df_lcox = pd.read_csv("cement/data/bau/def/intermediate/lcox.csv").set_index(
...
# private functions (pre-processing)
def _preprocess_capacity_factor(
    df_capacity_factor: pd.DataFrame, idx_per_input_metric: dict
) -> pd.DataFrame:

    # define idx
    idx_capacity_factor = [
        x
        for x in idx_per_input_metric["capacity_factor"]
        if x not in ["metric", "unit"]
    ]

    # filter relevant columns and reset index
    df_capacity_factor = df_capacity_factor.reset_index()[
        idx_capacity_factor + ["value"]
    ]
    df_capacity_factor = df_capacity_factor.set_index(idx_capacity_factor).sort_index()

    return df_capacity_factor


def _preprocess_plant_capacity_factor(
    df_plant_capacity: pd.DataFrame, idx_per_input_metric: dict
) -> pd.DataFrame:

    # define idx
    idx_plant_capacity = [
        x for x in idx_per_input_metric["plant_capacity"] if x not in ["metric", "unit"]
    ]

    # filter relevant columns and reset index
    df_plant_capacity = df_plant_capacity.reset_index()[idx_plant_capacity + ["value"]]
    df_plant_capacity = df_plant_capacity.set_index(idx_plant_capacity).sort_index()

    return df_plant_capacity
